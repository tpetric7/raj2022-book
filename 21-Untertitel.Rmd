...
Unsere erste Abbildung der Verhältnisse in den sechs Sprachen. 
...
ggsave("pictures/barplot_avatar_VO-OV_NN_PRON.png")
```

Die nächste Abbildung zeigt die Entfernung bzw. Ähnlichkeit der Sprachen ähnlich wie auf einer Landkarte an. 
...
ggsave("pictures/koord_avatar_VO-OV_NN_PRON.png")  
```


### Subjekt-vor-Objekt-Präferenz
...
Die nächste Funktion prüft, in welcher Reihenfolge Subjekt, Objekt und Verb auftreten. Unterschieden werden sechs verschiedene Konstituentenfolgen:   
...
Beide Tabellen werden zu einer vereint. 

```{r message=FALSE, warning=FALSE}
subject_object = bind_rows(svo_nominal, svo_pronominal) %>% 
  select(word_class, word_order:pct_tur)
subject_object %>% rmarkdown::paged_table()

```

In der ersten Abbildung verwenden wir liegende Säulendiagramme. Die relativ geringen Unterschiede zwischen den Sprachen waren zu erwarten, da nur SO-Sprachen ins Sample aufgenommen wurden. 
...
subject_object %>% 
  ...
  facet_wrap(~ language) +
  theme(legend.position = "top")
ggsave("pictures/barplot_avatar_SO-OS_NN_PRON.png")
```

Im zweiten Vergleich wählen wir wiederum eine landkartenähnliche Darstellungsweise. Da es sich um sechs Sprachen mit Subjekt-zuerst-Präferenz handelt, sind die Unterschiede relativ klein auf der Landkarte (besser zu sehen, wenn der Nullpunkt des Koordinaatensystems in die graphische Darstellung einbezogen ist - `expand_limits()`). 
...
ggsave("pictures/koord_avatar_SO-SO_NN_PRON.png")  
```


#### Belege für OS-Abfolgen
...
avatar_words_udpiped %>% 
  ...
  filter(word_order == "OS")
```

Slowenisch: OS-Abfolge mit Nomen oder Pronomen   
...
Deutsch: OS-Abfolge mit Pronomen   
...
Slowenisch: OS-Abfolge mit Nomen   
...
Slowenisch: OS-Abfolge mit Pronomen   
Besser. Aber einige Fehler in der Analyse. 
...
```{r}
# avatar_words_udpiped %>% filter(language == "deu")
```


```{r}
pivot_by_constituents <- function(tbl) {
  tbl %>% 
...
```{r}
pivot_by_constituents_which <- function(tbl){
  tbl %>% 
  filter(dep_rel %in% c("nsubj", "obj", "advmod", 
                        "aux", "cop", "root")) %>% 
  mutate(token_id_nsubj = ifelse(dep_rel %in% c("nsubj"), token_id, NA), 
...
avatar_words_udpiped %>% 
  ...
  ungroup() %>% 
  select(sentence, obj, nsubj, word_order, -language) %>% 
  filter(word_order %in% c("AaSOV", "AaSVO", "ASVO", "ASOV", "AVSO"))

```



#### Zusätzliche graphische Darstellungen 
